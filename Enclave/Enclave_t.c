...
typedef struct ms_encrypt_and_serialize_key_store_t {
	int ms_retval;
	void* ms_p_dst;
} ms_encrypt_and_serialize_key_store_t;
...
static sgx_status_t SGX_CDECL sgx_serialize_key_store(void* pms)
{
	...
	ms->ms_retval = serialize_key_store(_in_p_dst);
err:
	if (_in_p_dst) {
...
static sgx_status_t SGX_CDECL sgx_encrypt_and_serialize_key_store(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_encrypt_and_serialize_key_store_t));
	ms_encrypt_and_serialize_key_store_t* ms = SGX_CAST(ms_encrypt_and_serialize_key_store_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	void* _tmp_p_dst = ms->ms_p_dst;
	size_t _len_p_dst = 70;
	void* _in_p_dst = NULL;
	...
	ms->ms_retval = encrypt_and_serialize_key_store(_in_p_dst);
err:
	if (_in_p_dst) {
		memcpy(_tmp_p_dst, _in_p_dst, _len_p_dst);
		free(_in_p_dst);
...
SGX_EXTERNC const struct {
	...
	8,
	{
		...
		{(void*)(uintptr_t)sgx_encrypt_and_serialize_key_store, 0},
